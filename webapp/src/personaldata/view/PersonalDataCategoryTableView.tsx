...
// model
import { PersonalDataCategory, RewardCycleName } from '../../../../shared/personaldata/model/PerosnalData';
...
const PersonalDataCategoryTableView: StatelessComponent<PersonalDataCategoryTableViewProps> = props => {
  ...
  const columns = [
    ...
    },
    {
      ...
      maxWidth: 100,
      Cell: ({ value: rewardCycle }) => RewardCycleName[rewardCycle],
      Filter: ({ filter, onChange }) =>
        ...
        >
          ...
          {Object.keys(RewardCycleName).map(rewardCycle => {
            const rewardCycleName = RewardCycleName[rewardCycle];
            return (<option key={rewardCycleName} value={rewardCycleName}>{rewardCycleName}</option>)
          })}
        </select>,
      filterMethod: (filter, row) => {
        const filteringValue = filter.value;
        const rowValue = row[filter.id];
        return filteringValue === unfilteringConditionValue || filteringValue === RewardCycleName[rowValue];
      },
    },
    {
      id: 'etc',
